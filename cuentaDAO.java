...
public class cuentaDAO {
    ...
    private static String encontrarCuentaPorCorreo = "SELECT * FROM users WHERE username = ?";
    private static String updateDate = "UPDATE users set last_login = current_timestamp where username = ?";
 
    public boolean validarCuenta(cuentaVO cuenta) {
        ...
        PreparedStatement findPs = conn.prepareStatement(encontrarCuentaPorCorreo);
        PreparedStatement updatePs = conn.prepareStatement(updateDate);
        countPs.setString(1, cuenta.obtenerCorreo());
        findPs.setString(1, cuenta.obtenerCorreo());
        updatePs.setString(1, cuenta.obtenerCorreo());
        ...
        if(n == 1) {
            ...
            if (dbpwd.contentEquals(cuenta.obtenerContrasegna())) {
                updatePs.execute();
                result = true;
            }
        }
